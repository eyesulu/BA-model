...
def theoretical_dist_BA(k, m, cdf = False):
    if cdf == False:
...
def plot_m_dist (data, scale, num, m, color,show_title = True, cdf = False):
    if cdf == False:
        ...
        y_theory = theoretical_dist_BA(xs, m)
        ...
        xs = np.linspace (min(data), max(data), 1000)
        y_theory = theoretical_dist_BA(xs, m, cdf = True)
...
def stat_test(data, m, num, scale, sig, cdf = False, short_print = False):
    if cdf == False:
        x_binned, y_binned = lg.logbin(data, scale = scale)
        statistic, p_value = stats.kstest(y_binned, theoretical_dist_BA(x_binned, m) )
    else:
        x, y = getting_cum_prob(data)
        x = np.array(x)
        statistic, p_value = stats.kstest(y, theoretical_dist_BA(x, m, cdf = True) )
    if short_print == False:
        ...
        if p_value < sig:
            ...
            print ('>= %.2f' %sig)
def solve_quadratic(Ns, m, listofn = True):
    if listofn == True:
        answer = []
        for N in Ns:
...
def collapse(data, num, m, color ,scale, cdf = False):
    if cdf == False:
        ...
        x = x_binned / k1
        y = np.divide(y_binned,theoretical_dist_BA(x_binned, m, cdf = False))
        ...
        x = x_cdf / k1
        y = np.divide(y_cdf,theoretical_dist_BA(np.array(x_cdf), m, cdf = True))
        plt.loglog(x, y, '.', color=color, label = r'$N = %i, m = %.i$' %(num, m), alpha = 0.5)
        plt.xlabel (r'$k / k_1$')
...
y_th = theoretical_dist_BA(xs, m, cdf = False)
...
ms0 = BA_model_opt(ms[0])
ms1 = BA_model_opt(ms[1])
ms2 = BA_model_opt(ms[2])
ms3 = BA_model_opt(ms[3])
ms4 = BA_model_opt(ms[4])
ms5 = BA_model_opt(ms[5])
ms6 = BA_model_opt(ms[6])
...
axes.loglog(numx, solve_quadratic(numx, ms[0]),'-', color = blue, label = r'Theoretical $k_{max}$ for $m = %.i$' %ms[0])
...
axes.loglog(numx, solve_quadratic(numx, ms[5]),'-', color = red, label = r'Theoretical $k_{max}$ for $m = %.i$' %ms[5])
...
axes.loglog(numx, solve_quadratic(numx, ms[6]),'-', color = green, label = r'Theoretical $k_{max}$ for $m = %.i$' %ms[6])
...
axes.loglog(numx, solve_quadratic(numx, ms[2]),'-', color = purple, label = r'Theoretical $k_{max}$ for $m = %.i$' %ms[2])
