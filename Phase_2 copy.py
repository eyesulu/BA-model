...
def theoretical_dist_RA(k, m, cdf = False):
    if cdf == False:
        ...
        prob = -1/((m**m)*(1+m)**(1-m)) * (m/(1+m))**k / (m/(1+m) -1) 
        return prob
...
def getting_cum_prob(data):
    ...
    return values, y_cdf
def plot_m_dist (data, scale, num, m, color,show_title = True, cdf = False):
    if cdf == False:
        ...
        plt.ylabel(r'PDF')
        if show_title == True:
            plt.title(r'Degree distribution for N = %i, scale = %.2f' %(num, scale))
    else:
        ...
        if show_title == True:
            plt.title(r'Degree distribution for N = %i, scale = %.2f' %(num, scale))
def plot_N_dist(data, scale, num, m, color,show_title = True, plot_th = False, cdf = False): 
    if cdf == False:
        x, y = getting_prob(data)
        x_binned, y_binned = lg.logbin(data, scale = scale)
        xs = np.linspace (min(data), max(data), 1000)
        ...
        plt.ylabel(r'CCDF')
        if show_title == True:
            plt.title(r'Degree distribution for m = %i, scale = %.2f' %(m, scale))
    
        
def stat_test(data, m, num, scale, sig, cdf = False, short_print = False):
    if cdf == False:
        x_binned, y_binned = lg.logbin(data, scale = scale)
    ...
    if short_print == False:
        print ('The p_value for the theoretical distribution is %.3f for m = %i, N = %i, scale = %.2f' 
               %(p_value, m , num, scale))
        if p_value < sig:
            print ('It is < %.2f hence the distribution does not follow the theoretical predictions' %sig)
        else:
            print ('It is >= %.2f hence the distribution does follow the theoretical predictions' %sig)
    else:
        if p_value < sig:
            print ('< %.2f' %sig)
        else:
            print ('>= %.2f' %sig)
def exp_kmax(Ns, m, listofn = True):
    if listofn == True:
        answer = []
        for N in Ns:
...
def collapse(data, num, m, color ,scale, cdf = False):
    if cdf == False:
        x_binned, y_binned = lg.logbin(data, scale = scale)
        k1 = solve_quadratic(num, m, listofn = False)
        x = x_binned / np.max(x_binned)
        # x = x_binned / k1
        ...
        k1 = solve_quadratic(num, m,listofn = False )
        x = x_cdf / np.max(x_cdf)
        # x = x_cdf / k1
        y = np.divide(y_cdf,theoretical_dist_RA(np.array(x_cdf), m, cdf = True))
        plt.loglog(x, y, '-', color=color, label = r'$N = %i, m = %.i$' %(num, m), alpha = 0.5)
        plt.xlabel (r'$k / k_1$')
        plt.ylabel (r'$p_{>} (k) / p_{> , \infty} (k)$')
def overflow(data, scale, color ):
    x, y = getting_prob(data)
    x_binned, y_binned = lg.logbin(data, scale = scale)
    xs = np.linspace (min(data)-min(data), max(data)-min(data), 1000)
    def approx(k, m):
        return a + k*(m/(m+1))
    y_theory = theoretical_dist_RA(xs, m)
    plt.loglog(x_binned, y_binned,'.', color = color, alpha = 0.5)
    plt.loglog(xs+min(data), y_theory,'--',linewidth = 0.8, label =r'Theoretical dist. for $m = %i$' %m, color = color) 
    plt.legend()
    plt.grid(False)
    plt.xlabel(r'$ k$')
    plt.ylabel(r'PDF')
...
data6 = ms6.degrees()
#%%
plot_m_dist(data0, 1.1, num, ms[0], red)
...
#%%
overflow(data6, 1.1, red)
#%%
plot_m_dist(data0, 1.1, num, ms[0], red, cdf = True)
plot_m_dist(data1, 1.1, num, ms[1], blue, cdf = True)
plot_m_dist(data2, 1.1, num, ms[2], green, cdf = True)
plot_m_dist(data3, 1.1, num, ms[3], purple, cdf = True)
plot_m_dist(data4, 1.1, num, ms[4], pink, cdf = True)
plot_m_dist(data5, 1.1, num, ms[5], yellow, cdf = True)
plot_m_dist(data6, 1.1, num, ms[6], grey, cdf = True)
#%%
'''
...
'''
#%%
collapse(data0, num, ms[0], red,scale = 1.1, cdf = False)
collapse(data1, num, ms[1], blue,scale = 1.1, cdf = False)
collapse(data2, num, ms[2], green,scale = 1.1, cdf = False)
collapse(data3, num, ms[3], purple,scale = 1.1, cdf = False)
collapse(data4, num, ms[4], pink,scale = 1.1, cdf = False)
collapse(data5, num, ms[5], yellow,scale = 1.1, cdf = False)
collapse(data6, num, ms[6], grey,scale = 1.1, cdf = False)

collapse(datan0, nums[0], m, 'red',scale = 1.1, cdf = False)
collapse(datan1, nums[1], m, 'blue',scale = 1.1, cdf = False)
collapse(datan2, nums[2], m, 'green',scale = 1.1, cdf = False)
collapse(datan3, nums[3], m,'purple',scale = 1.1, cdf = False)
collapse(datan4, nums[4], m, 'pink',scale = 1.1, cdf = False)
collapse(datan5, nums[5], m, 'yellow',scale = 1.1, cdf = False)
collapse(datan6, nums[6], m, 'grey',scale = 1.1, cdf = False)
